    ...
    try:
        ...
        for jogo in data:
            if jogo["color"] == 1:
                ...
                cor = "Desconhecido"
            resultados.append({"cor": cor, "numero": jogo["roll"]})
        return resultados
    except Exception as e:
        return []

def prever_cor(resultados):
    if not resultados or len(resultados) < 3:
        return "Indefinido"
    
    ultimos = [r["cor"] for r in resultados[:3] if r["cor"] != "Branco"]
    
    if not ultimos:
        return "Indefinido"
    
    vermelho_count = ultimos.count("Vermelho")
    preto_count = ultimos.count("Preto")
    ...
    if vermelho_count >= 2:
        ...
        # Se não houver maioria, escolhe a cor oposta da última
        return "Vermelho" if ultimos[0] == "Preto" else "Preto"
...
while True:
    with placeholder.container():
        resultados = obter_ultimos_resultados()
        cor_indicada = prever_cor(resultados)
        ...
        st.markdown(f"""
        <div style="
            background: {cores_bg.get(cor_indicada, '#6b7280')};
            color: {'#000000' if cor_indicada == 'Branco' else '#ffffff'};
            padding: 15px;
            border-radius: 8px;
            text-align: center;
            font-size: 1.5rem;
        ...
        if resultados:
            ...
            st.markdown(" ".join(
                "🔴" if r["cor"] == "Vermelho" else "⚫" if r["cor"] == "Preto" else "⚪"
                for r in resultados
            ))
        else:
